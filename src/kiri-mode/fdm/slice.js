...
FDM.slice = function(settings, widget, onupdate, ondone) {
    ...
    Promise.all([loadedData3, loadedData2]).then((val) => {
        ...
        // prisms_obj.push({geometry_points:parseSVGFromText(val[1]), name:"silver_jack", ini_height:20.75, extension_range:17});
        // prisms_obj.push({geometry_points:parseSVGFromText(val[1]), name:"silver_jack", ini_height:5, extension_range:10});
        // prisms_obj.push({geometry_points:parseSVGFromText(val[1]), name:"blue_jack", ini_height:5, extension_range:10});
    })
    ...
    async function onSliceDone(slices) {
        ...
        if (!isBelt && !isSynth && supportDensity && process.sliceSupportEnable) {
            ...
            if (process.surrogateInteraction != "none") {
                ...
                console.log({new_bottom: prepared_slices[0], old_bottom:bottom_slice});
                ...
                let all_slices_save = [...surrogate_settings.all_slices];
                ...
                if (false) {
                    var meta_initialPopulationSize = 15;
                    var meta_iterations = 300;
                    var meta_domain = [
                        new kiri.Interval(0.5, 1.0)
                    ];

                    var meta_optimizer = new kiri.Optimizer();

                    console.log({interval:new kiri.Interval(0, 1)});

                    meta_optimizer.init(meta_initialPopulationSize, meta_domain);

                    let workaroundBottomSlice = bottom_slice;
                    meta_optimizer.setObjectiveFunction(function (x, done) {
                        // var pso_params = {
                        //     inertiaWeight: x[0],
                        //     social: x[1],
                        //     personal: x[2]
                        ...
                        for (let cluster of cluster_list) {
                            ...
                            for (let cluster_hull of cluster.concave_cluster_hulls) {
                                // let data = kiri.minions.surrogateClusterSearch(sliceStackData, surrogate_library, cluster_hull, surrogate_settings, pso_params, settings.device, bottom_slice.widget, cluster.kn-1, clusterID); // kn-1 because we plus1 them when building clusters, but susu_data_objs counting starts at 0
                                clusterID += 1;

                                sliceStackData = getEncodedData(workaroundBottomSlice);
                                let slice_stack_data = sliceStackData;
                                let hull_points = cluster_hull;
                                let device = settings.device;
                                // let susu_settings = surrogate_settings;
                                // let pso_params = pso_params;
                                let t_pso_params = pso_params;
                                let widget = workaroundBottomSlice.widget;

                                var optimizer = new kiri.Optimizer();
                                // let { slice_stack_data, surrogate_library, hull_points, susu_settings, pso_params, t_pso_params, device, widget } = data;
                                ...
                                for (let slice_data of slice_stack_data) {
                                    let mock_slice = kiri.codec.decode(slice_data[0]);
                        
                                    for (let oneTop of slice_data[1]) {
                                        let oneDecodedTop = kiri.codec.decode(oneTop, {full: false});
                                    ...
                                    // }
                                    if (slice_data[2].length > 0) mock_slice.supports = kiri.codec.decode(slice_data[2]);
                                ...
                                surrogate_settings.all_slices = mock_slice_list;
                                ...
                                optimizer.setOptions({
                                    inertiaWeight: pso_params.inertiaWeight,
                                    social: pso_params.social,
                                    personal: pso_params.personal
                                });
                                ...
                                function loop() {
                                    if (iterations >= maxIterations) { // TODO: Need handling of further execution if this case is reached
                                        ...
                                        let meanFitness = optimizer.getBestFitness();
                                        ...
                                        if (!Number.isFinite(meanFitness)) meanFitness = 0;
                                        sumMean = meanFitness;
                                        // console.log({meanFit:meanFitness});
                                        ranX++;
                                        // console.log({fitness:stepFitness});
                                ...
                                t_optimizer.setOptions({
                                    inertiaWeight: t_pso_params.inertiaWeight,
                                    social: t_pso_params.social,
                                    personal: t_pso_params.personal
                                });
                                ...
                                // let val = { output:return_list.length, return_list:return_list, history_list, ranX: ranX, sumMean:sumMean };

                                
                                meta_fitness += (sumMean / 1.0);
                                // console.log({sumMean:sumMean, ranX:ranX, meta_fitness:meta_fitness});
                        ...
                        //     let clusterID = 0;
                        //     for (let cluster_hull of cluster.concave_cluster_hulls) {
                        //         optimizer_promises.push(kiri.minions.surrogateClusterSearch(sliceStackData, surrogate_library, cluster_hull, surrogate_settings, pso_params, settings.device, bottom_slice.widget, cluster.kn-1, clusterID)); // kn-1 because we plus1 them when building clusters, but susu_data_objs counting starts at 0
                        //         clusterID += 1;
                        //     }
                        // }
                        ...
                        //             meta_fitness += (data.sumMean / data.ranX);
                    ...
                    let performanceList = [];
                    var metaIterations = 0;
                    function metaLoop() {
                        if (metaIterations >= meta_iterations) { // TODO: Need handling of further execution if this case is reached
                            let stepFitness = meta_optimizer.getBestFitness();
                            let stepPara = meta_optimizer.getBestPosition();
                            console.log({fitness:stepFitness});
                            performanceList.push({parameters:stepPara, result:stepFitness});
                            performanceString += stepFitness.toString() + "\n";
                        } else {

                            metaIterations++;
                            console.log('Iteration ' + metaIterations);

                            let stepFitness = meta_optimizer.getBestFitness();
                            let stepPara = meta_optimizer.getBestPosition();
                            console.log({fitness:stepFitness});
                            if (Number.isFinite(stepFitness)) {
                                console.log({stepPara0:stepPara[0]});
                                console.log({stepPara1:stepPara[1]});
                                console.log({stepPara2:stepPara[2]});
                                console.log({stepPara3:stepPara[3]});
                            }
                            console.log(performanceString);
                            performanceList.push({parameters:stepPara, result:stepFitness});
                            performanceString += stepFitness.toString() + "\n";

                            meta_optimizer.step(metaLoop);
                        }
                    }
                ...
                // let pso_params = {inertiaWeight: 0.7,
                //     social: 0.6,
                //     personal: 0.5,
                //     pressure: 0.5};

                // let t_pso_params = {inertiaWeight: 0.7,
                //     social: 0.6,
                //     personal: 0.5,
                //     pressure: 0.5};
                ...
                if (optimizer_promises) {
                    for (let p of optimizer_promises) {
                        p.then(data => { 
                            ...
                            if (surrogate_settings.visualize_output) all_histories.push({clusterKN:data.kn, clusterID:data.clusterID, historyList: decoded_histories_list})
                    ...
                    for (let i = 0; i < 3; i++) {
                        if (global_selection_list[i].final_fitness < data_obj.selection_list[i].final_fitness) {
                            global_selection_list[i] = {
                                ...
                                complexity_of_interactions: data_obj.selection_list[i].complexity_of_interactions,
                                best_kn: data_obj.kn
                            };
                        }
                    }
                ...
                // Making surrogate objects from search result
                ...
                let skip = true;
                if (global_selection_list[0].result_fitness) skip = false;
                ...
                if (!skip) {
                    ...
                    if (surrogate_settings.visualize_output) {
                        let exportPolys = [];
                        for (let topPoly of higherSlice.topPolys()) {
                            let pointString = "";
                        ...
                        for (let supportPoly of higherSlice.supportsSaved) {
                            let pointString = "";
                ...
                bottom_slice.efficiencyData = {numberPauses: 0, numberSurrogates: 0, materialWeightEstimateTube: 0, materialWeightEstimateBar: 0, materialWeightEstimateEllipse: 0, timestamp:0, id:bottom_slice.widget.id, previous_volume:0, new_volume:0, volume_percentage_saved:0, sTime:startTime,
                    ...
                    // numberPausesHigh:0,
                    // FitnessHigh:0,
                    // numberSurrogatesMedium:0,
                    // numberPausesMedium:0,
                    // FitnessMedium:0,
                    // numberSurrogatesLow:0,
                    // numberPausesLow:0,
                    // FitnessLow:0,
                    numberSurrogatesX:0,
                    numberPausesX:0,
                    Fitness:0,
                    searchQual:"Search Offline",
                    interactionLevel:"No Surrogates",
                    towers:false
                }
            }
    ...
    function placeSurrogates(global_selection_list, surrogate_settings, surrogate_library, bottom_slice, surrogates_placed) {
        ...
        function makeCandidate(cd) {
            let candidate_detail = cd.candidate_details;
            ...
            if (candidate_detail.use_me) { // If surrogate was placed without problem by PSO
                ...
                surrogates_placed.forEach(function(surrogate) {
                    if (Math.abs(candidate_detail.candidate_obj.insertion_data.index - surrogate.insertion_data.index) === 1) {
                        consecutive = true;
                    }
                });
                if (consecutive) {
                ...
                if (candidate_detail.candidate_obj.surro.available === false) {
                    console.log({Warning_note:"A surro has been chosen that was not available."});
                }

                surrogates_placed.push(candidate_detail.candidate_obj);
                candidate_detail.candidate_obj.surro.available = false; // Mark surro as used

                // console.log({placed_surro_name:candidate_detail.candidate_obj.surro.id});

                // Try skipping this stuff and using delivered details instead
                // if (candidate_detail.candidate_obj.surro.type !== "simpleRectangle") {

                let try_x = candidate_detail.pso_details[0];
                let try_y = candidate_detail.pso_details[1];
                let try_rotation = candidate_detail.candidate_obj.rotation;
                let try_inner_rot = candidate_detail.pso_details[5];
                ...
                // let pso_desired_width = candidate_detail.pso_details[4];
                // let pso_desired_height = candidate_detail.pso_details[5];

                // try_surro = getBestFittingSurrogateL2(surros, pso_desired_length, pso_desired_width, pso_desired_height);
                let try_surro = candidate_detail.candidate_obj.surro;

                let try_z = candidate_detail.candidate_obj.starting_height;

                // Adjust position/rotation to lower surrogate in tower
                if (candidate_detail.tower_details.tower_x !== null) {
                    is_tower = true;
                    // console.log({tower_details:candidate_detail.tower_details});
                    try_rotation = candidate_detail.tower_details.tower_rot;
                    try_x = candidate_detail.tower_details.tower_x;
                    try_y = candidate_detail.tower_details.tower_y;
                }
                ...
                let id_extension = candidate_detail.candidate_obj.end_height - try_z;
                id_extension = '_' + id_extension + 'mm';
                ...
                if (try_surro.type == "simpleRectangle") {
                    ...
                    if (is_tower) prism_bottoms = [generatePrismPolygonCentered(try_x, try_y, try_z, try_surro.bottom_geometry, try_rotation, try_inner_rot, surrogate_settings.surrogate_padding, bottom_slice)]; // TODO align these using SVG viewbox
                    else prism_bottoms = [generatePrismPolygon(try_x, try_y, try_z, try_surro.bottom_geometry, try_rotation, try_inner_rot, surrogate_settings.surrogate_padding, bottom_slice)]; // TODO align these using SVG viewbox
                    ...
                    // console.log({note:"CompareStack2", pso_polygons_list:pso_polygons_list, geo:candidate_detail.candidate_obj.geometry});
                ...
                console.log("Warning: Surrogate was set to not use?");
            }
        }
        ...
        if (surrogate_settings.surrogateInteraction == "medium") selection_index = 1;
        else if (surrogate_settings.surrogateInteraction == "high") selection_index = 2;
        let selection = global_selection_list[selection_index];
        for (let candidate of selection.used_candidates) {
            let single_candidate = makeCandidate(candidate.cd);
            let tower_candidates = [];
            tower_candidates.push(single_candidate);
            // console.log({candidate:candidate});
            if (candidate.tower_step_selection > 0) {
                for (let above_cd of candidate.cd.aboves[candidate.tower_step_selection]) {
                    let above_candidate = makeCandidate(above_cd);
                    tower_candidates.push(above_candidate);
                }
            }
            tower_candidates.sort(function(a, b) { // Sort by end height
                var keyA = a.candidate_details.candidate_obj.end_height,
                    keyB = b.candidate_details.candidate_obj.end_height;
                if (keyA < keyB) return -1;
                if (keyA > keyB) return 1;
                return 0;
            });

            // console.log({tower_candidates:tower_candidates});
            
            if (tower_candidates.length > 1) {
                for (let tower_candidate = 0; tower_candidate < tower_candidates.length; tower_candidate++) {
                    // console.log({tower_candidate:tower_candidate});
                    if (tower_candidate > 0) {
                        tower_candidates[tower_candidate-1].candidate_details.candidate_obj.up_surrogate.push(tower_candidates[tower_candidate].candidate_details.candidate_obj);
                        tower_candidates[tower_candidate].candidate_details.candidate_obj.down_surrogate.push(tower_candidates[tower_candidate-1].candidate_details.candidate_obj);
                    }
                }
            }
            

            all_candidates.push(...tower_candidates);
        }
    ...
    function applySurrogatesToSlices(surrogates_placed, surrogate_settings, bottom_slice, all_histories, proc, view, all_slices, pre_surrogate_support_amounts, global_selection_list, startTime, skip) {
        ...
        let post_surrogate_support_amounts = null;
        let volume_percentage_saved = 0;
        let all_out_slices = [];
        ...
        if (!skip) {
            ...
            if (surrogate_settings.surrogateInteraction !== "none") {
                    ...
                    for (let draw_outline_idx = 0; draw_outline_idx < surrogates_placed.length; draw_outline_idx++) {
                        ...
                        if (up.z < (surrogate.end_height) && up.z >= surrogate.starting_height) {
                            if (surrogate.outlines_drawn < 6) {
                                ...
                                surrogate_enlarged = POLY.expand(geometry_to_use, (surrogate_settings.lineWidth/2) - surrogate_settings.surrogate_padding, up.z, surrogate_enlarged, 1); // 0.1 for prism. TODO: Match padding to this
                                surrogate_double_enlarged = POLY.expand(surrogate_enlarged, surrogate_settings.lineWidth, up.z, surrogate_double_enlarged, 1); 
                                ...
                                //if (!first_placed) { // Switch mode for first outline
                                    ...
                                    if (surrogate.outlines_drawn < 1) {
                                        /*
                                        let extra_outlines = [];
                                        let extra_outlines_lines = [];
                                        extra_outlines = POLY.expand(surrogate_outline_area_only, 0.45, up.z, extra_outlines, 1);
                                        POLY.subtract(extra_outlines, surrogate_enlarged, extra_outlines_lines, null, up.z, 3);
                                        extra_outlines_lines[0].points.push(extra_outlines_lines[0].points[0]);
                                        up.tops[0].fill_sparse.push(extra_outlines_lines[0]);
                                        */

                                        
                                        for (let brim_lines = 1; brim_lines <= surrogate_settings.brim_size; brim_lines++) {
                                            let extra_outlines = [];
                                            let extra_outlines_inner = [];
                                            let extra_outlines_done = [];
                                            extra_outlines = POLY.expand(surrogate_outline_area_only, 0.45*(brim_lines), up.z, extra_outlines, 1);
                                            if (brim_lines > 1) {
                                                extra_outlines_inner = POLY.expand(surrogate_outline_area_only, 0.45*(brim_lines-1), up.z, extra_outlines_inner, 1);
                                                POLY.subtract(extra_outlines, extra_outlines_inner, extra_outlines_done, null, up.z, 3);
                                            } else {
                                                POLY.subtract(extra_outlines, surrogate_outline_area_only, extra_outlines_done, null, up.z, 3);
                                            }
                                    ...
                                    if (surrogate.outlines_drawn < 1 && surrogate.surro.type == "prism") {
                                        ...
                                        surrogate_enlarged2 = POLY.expand(surrogate.geometry, 0.2, up.z, surrogate_enlarged2, 1);
                        ...
                        // Simple cases: One surrogate on the slice
                            ...
                            if (surrogates_at_this_index[0].insertion_data.insertion_case === "Insert_new_support_layer") {
                                ...
                                if (collect_all_polygons_removed_from_support.length > 0) {
                                    POLY.subtract(original_supports, collect_all_polygons_removed_from_support, only_support_above_this_surrogate_2, null, new_layer_height_range.bottom_height, 3);
                                } else {
                                    only_support_above_this_surrogate_2 = original_supports;
                                }
                                if (only_support_above_this_surrogate_2.length > 0) {
                                    POLY.subtract(only_support_above_this_surrogate_2, collect_all_surrogate_geometries_removed_from_support, only_support_above_this_surrogate, null, new_layer_height_range.bottom_height, 3);
                                } else {
                ...
                for (let draw_pause_text_idx = 0; draw_pause_text_idx < surrogates_placed.length; draw_pause_text_idx++) { // TODO reactivate
                    ...
                    if (true) { 
                        ...
                        text_posX_alt = text_posX_alt - Math.cos(text_rot_ang)*(currentSurrogate.surro.length*0.5) + Math.sin(text_rot_ang)*(currentSurrogate.surro.width*0.5+surrogate_settings.char_size+3.5);
                        text_posY_alt = text_posY_alt - Math.sin(text_rot_ang)*(currentSurrogate.surro.length*0.5) - Math.cos(text_rot_ang)*(currentSurrogate.surro.width*0.5+surrogate_settings.char_size+3.5);
                    ...
                    if (!topOrBottom && tower_surrogates.length > 0) {
                        text_posX = text_posX + Math.sin(text_rot_ang)*(surrogate_settings.char_size*2)*(tower_surrogates.length);
                        text_posY = text_posY - Math.cos(text_rot_ang)*(surrogate_settings.char_size*2)*(tower_surrogates.length);
                    }

                    let id_ext = "";
                    if (currentSurrogate.surro.type == "prism" || currentSurrogate.surro.type == "stackable") id_ext = "_" + Math.round(currentSurrogate.end_height - currentSurrogate.starting_height) + "mm";
                    console.log({currentSurrogate:currentSurrogate});
                    const firstString = "P" + (surrogate_settings.pauseLayers.indexOf(currentSurrogate.insertion_data.printed_layer_index) + 1).toString() + " " +currentSurrogate.surro.id + "" +id_ext;
                    ...
                    tower_surrogates.forEach(function(upSupp) {
                        if (true) {
                            text_posX = text_posX + Math.sin(text_rot_ang)*(-surrogate_settings.char_size*2);
                            text_posY = text_posY - Math.cos(text_rot_ang)*(-surrogate_settings.char_size*2);
                        ...
                        let id_ext = "";
                        if (upSupp.surro.type == "prism" || upSupp.surro.type == "stackable") id_ext = "_" + Math.round(upSupp.end_height - upSupp.starting_height) + "mm";
                        const additionalString = "P" + (surrogate_settings.pauseLayers.indexOf(upSupp.insertion_data.printed_layer_index) + 1).toString() + " " +upSupp.surro.id+ "" +id_ext;
            ...
            post_surrogate_support_amounts = getTotalSupportVolume(bottom_slice);
            // console.log({post_surrogate_support_amounts:post_surrogate_support_amounts});

            let volume_saved = 0;
            if (post_surrogate_support_amounts) volume_saved = pre_surrogate_support_amounts[0] - post_surrogate_support_amounts[0];
            
            volume_percentage_saved = volume_saved / pre_surrogate_support_amounts[0];
        ...
        if(!global_selection_list[0].result_fitness) global_selection_list[0].result_fitness = 0;
        if(!global_selection_list[1].result_fitness) global_selection_list[1].result_fitness = 0;
        if(!global_selection_list[2].result_fitness) global_selection_list[2].result_fitness = 0;
        if(!global_selection_list[0].number_of_surrogates) global_selection_list[0].number_of_surrogates = 0;
        if(!global_selection_list[1].number_of_surrogates) global_selection_list[1].number_of_surrogates = 0;
        if(!global_selection_list[2].number_of_surrogates) global_selection_list[2].number_of_surrogates = 0;
        if(!global_selection_list[0].number_of_interactions) global_selection_list[0].number_of_interactions = 0;
        if(!global_selection_list[1].number_of_interactions) global_selection_list[1].number_of_interactions = 0;
        if(!global_selection_list[2].number_of_interactions) global_selection_list[2].number_of_interactions = 0;

        let new_volume = pre_surrogate_support_amounts[0];
        if (post_surrogate_support_amounts) new_volume = post_surrogate_support_amounts[0];

        const efficiencyData = {numberPauses: surrogate_settings.pauseLayers.length, numberSurrogates: surrogates_placed.length, materialWeightEstimateTube: 0, materialWeightEstimateBar: 0, materialWeightEstimateEllipse: 0, timestamp:bottom_slice.widget.surrogate_data.timestamp, id:bottom_slice.widget.id, previous_volume:pre_surrogate_support_amounts[0], new_volume:new_volume, volume_percentage_saved:volume_percentage_saved, sTime:sTime,
            ...
            numberSurrogatesX:global_selection_list[0].number_of_surrogates,
            numberPausesX:global_selection_list[0].number_of_interactions,
            Fitness:global_selection_list[0].result_fitness,
            searchQual:surrogate_settings.surrogateSearchQual,
            interactionLevel:surrogate_settings.surrogateInteractionText,
            towers:surrogate_settings.allow_towers,
            interactionComplexity:global_selection_list[0].complexity_of_interactions
    ...
    function getSurrogateLibrary(prisms) {
        ...
        addOption(surrogates, 137.46, 54.42, 6.69, "wood plate");
        // addOption(surrogates, 208.8, 164, 6.66, "wood plate large");
        // addOption(surrogates, 93.8, 89.9, 6.66, "floppyx2");
        addOption(surrogates, 128.35, 68.2, 8.73, "medium dense foam plate");

        addOption(surrogates, 73, 10.43, 9.5, "support bar");
        // addOption(surrogates, 93.8, 89.9, 9.99, "floppyx3");
        addOption(surrogates, 44.35, 18.35, 10.16, "wood bar4"); // addOption(surrogates, 44.35, 18.33, 10.16, "wood bar dirty");
        ...
        addOption(surrogates, 45.3, 23.35, 14.8, "dark wood"); // higer on one side
        ...
        addOption(surrogates, 52.22, 24.95, 18.62, "wood bar");
        ...
        addOption(surrogates, 24.25, 24.25, 24.08, "XYZ cube filled");
        ...
        addOption(surrogates, 170, 144.0, 37.6, "scale box");
        ...
        // addStackableOptions(surrogates, 3.33, 3.33, 8, 93.8, 89.9, "FloppyDisc");
        ...
        */
        ...
        // addOption(surrogates, 75.1, 73, 29.05, "lowDensityFoam");
        // addOption(surrogates, 183.5, 80.1, 14.7, "foam plate");
        // addOption(surrogates, 137.46, 54.42, 6.69, "wood plate");
        // addOption(surrogates, 128.35, 68.2, 8.73, "medium dense foam plate");
        // addOption(surrogates, 52.22, 24.95, 18.62, "wood bar");
        // addOption(surrogates, 44.35, 18.35, 10.16, "wood bar4");
        // addOption(surrogates, 45.3, 23.35, 14.8, "dark wood"); // higer on one side
        // addOption(surrogates, 170, 144.0, 35.6, "scale box");
        ...
        // addOption(surrogates, 31.89, 31.89, 22.3, "  ");

        // addOption(surrogates, 24.15-0.1, 24.15-0.1, 23.9, "   "); //  Purple

        // addOption(surrogates, 142, 210, 23.2, "   "); //  softcover book
        // addOption(surrogates, 20, 20, 24.7, "   "); //  hardcover book
        // addOption(surrogates, 31.8, 31.8, 22.25, "   ");
    ...
    // 1: Lego // 111000 = fist size block // 3000000 a lot
    ...
    function prepareSurrogating(library_in, slice, proc, settings) {
        ...
        surrogate_settings.lineWidth = proc.sliceLineWidth || settings.device.extruders[parseInt(isSynth ? process.sliceSupportNozzle : metadata.extruder || 0)].extNozzle,

        // Have all settings available in parallel
        surrogate_settings.surrogate_N_penalty_factor_high = 0.0; 
        surrogate_settings.interaction_N_penalty_factor_high = surrogate_settings.surrogate_N_penalty_factor_high*0.3;
        

        surrogate_settings.surrogate_N_penalty_factor_med = 0.5; 
        surrogate_settings.interaction_N_penalty_factor_med = surrogate_settings.surrogate_N_penalty_factor_med*0.3;
        
        
        surrogate_settings.surrogate_N_penalty_factor_low = 1.0;
        surrogate_settings.interaction_N_penalty_factor_low = surrogate_settings.surrogate_N_penalty_factor_low*0.3;
        ...
        if (proc.surrogateInteraction == "none") {
            ...
            surrogate_settings.surrogateInteractionText = "Low Interaction";
        } else if(proc.surrogateInteraction == "medium") {
            ...
            surrogate_settings.surrogateInteractionText = "Medium Interaction";
        } else if(proc.surrogateInteraction == "high") {
        ...
        if (proc.surrogateSearchQual == "fastest") {
            surrogate_settings.minVolume = 120;
            surrogate_settings.exploration_factor = 0.3;
            // surrogate_settings.simplification_factor = 4.0;
            surrogate_settings.search_persistance = 3;
            surrogate_settings.minImprovementPercentage = 0.1;
            surrogate_settings.numberOfParticles = 125;
            surrogate_settings.towerParticles = 8;
            surrogate_settings.max_search_iterations = 12;
            surrogate_settings.max_tower_iterations = 4;
            surrogate_settings.searchspace_max_number_of_surrogates = 4;
            surrogate_settings.surrogateSearchQual = "Fast Search";
            
        } else if(proc.surrogateSearchQual == "fair") {
            surrogate_settings.minVolume = 100;
            surrogate_settings.exploration_factor = 0.25;
            // surrogate_settings.simplification_factor = 3.5;
            ...
            surrogate_settings.towerParticles = 10;
            surrogate_settings.max_search_iterations = 15;
            surrogate_settings.max_tower_iterations = 4;
            surrogate_settings.searchspace_max_number_of_surrogates = 5;
            surrogate_settings.surrogateSearchQual = "Medium Search";

        } else if(proc.surrogateSearchQual == "good") {
            surrogate_settings.minVolume = 80;
            ...
            surrogate_settings.minImprovementPercentage = 0.06;
            surrogate_settings.numberOfParticles = 225;//225
            surrogate_settings.towerParticles = 12;
            surrogate_settings.max_search_iterations = 25;
            surrogate_settings.max_tower_iterations = 4;
            surrogate_settings.searchspace_max_number_of_surrogates = 6;
            surrogate_settings.surrogateSearchQual = "Thorough Search";

        } else if(proc.surrogateSearchQual == "best") {
            surrogate_settings.minVolume = 60;
            surrogate_settings.exploration_factor = 0.1;
            // surrogate_settings.simplification_factor = 2.5;
            surrogate_settings.search_persistance = 6;
            surrogate_settings.minImprovementPercentage = 0.04;
            surrogate_settings.numberOfParticles = 275;//275;
            surrogate_settings.towerParticles = 14;
            surrogate_settings.max_search_iterations = 45;
            surrogate_settings.max_tower_iterations = 5;
            surrogate_settings.searchspace_max_number_of_surrogates = 7;
            surrogate_settings.surrogateSearchQual = "Best Search";
        }

        surrogate_settings.inertiaWeightS = 0.7; // Hyperparameter optimization results
		surrogate_settings.socialS = 0.76;//0.6
		surrogate_settings.personalS = 0.66;//0.5
		surrogate_settings.pressureS = 0.5;
        surrogate_settings.annealingExponent = 0.95;
        ...
        surrogate_settings.brim_size = proc.surrogateBrimSize;

        surrogate_settings.text_size = proc.surrogateTextSize;
        surrogate_settings.char_size = surrogate_settings.text_size/20*4.5;
        ...
        while (up) {
            ...
            if (up.supports) {
                let unionized_supports = POLY.union(up.supports, 10, true);
            ...
            // if (up.supports) {
            //     if (up.supports.length > 10) {
            //         up.supports.splice(10, up.supports.length-10);
            //     }
            // }

            if (up.supports) { // Union supports again after simplify
                let unionized_supports = POLY.union(up.supports, 10, true);
                // console.log({unionized_supports:unionized_supports});
            ...
            // if (up.supports) { // Union supports again after simplify
            //     let unionized_supports = POLY.union(up.supports, 5, true);
            //     // console.log({unionized_supports:unionized_supports});
            ...
            // }
            // if (up.supports) { // Union supports again after simplify
            //     let unionized_supports = POLY.union(up.supports, 0.0000001, false);
            //     // console.log({unionized_supports:unionized_supports});
        ...
        return [ all_slices, surrogate_settings, pre_surrogate_support_amounts ];
    }
