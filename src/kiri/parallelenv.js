(function() {
    if (self.kiri.parallelenv) return;
  
    self.kiri.parallelenv = {
        Operation: Operation,
        Parallel: Parallel
    };
    ...
    // console.log({dir:`${__dirname}/Worker-parallel.js`});
    var setImmediate =
      setImmediate ||
      function(cb) {
        setTimeout(cb, 0);
      };
    const Worker = isNode ? require(`${__dirname}/Worker-parallel.js`) : self.Worker;
    const URL =
