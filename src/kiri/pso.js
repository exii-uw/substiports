...
// dep: geo.base
gapp.register("kiri.pso", [], (root, exports) => {

const { base, kiri } = root;

// let PARALLELENV = kiri.parallelenv
...
Optimizer.prototype = {
	...
	step: function (callback) {
		if (this._async) {
			// console.log({status:"Async objective function"});
			if (this._waiting) {
			...
			// let parallelTest = new PARALLELENV.Parallel(this._particles, {
			// 	env: {
			// 	  a: 10
			// 	},
			...
			// function one_particle_step(particle) {
			// 	console.log({particle:particle});
			// 	particle.fitness = this._objectiveFunction(particle.position);
			// };

			// const log_and_continue = function () { 
			// 	console.log(arguments);
			// 	this._waiting = false;
			// 	this._completeStep();
			// };

			// parallelTest.map(one_particle_step).then(log_and_continue)
